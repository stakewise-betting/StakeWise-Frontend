...
import UserManagementPage from "@/Admin/users/UserManagementPage";
import RafflesPage from "@/Admin/raffles/RafflesPage"; // This component is now self-contained
import SliderPage from "./slider/slider";
import setupWeb3AndContract from "@/services/blockchainService";
import { Skeleton } from "@/components/ui/skeleton";
import { Button } from "@/components/ui/button";
import { RecentBet } from "@/Admin/dashboard/RecentBetsTable";
...
const AdminPanel: React.FC = () => {
  ...
  // --- REMOVED --- Obsolete raffle contract state
  // const [raffleContract, setRaffleContract] = useState<any>(null); 
  const [adminProfit, setAdminProfit] = useState<string>("0");
  const [adminAddress, setAdminAddress] = useState<string>("");
  ...
  const loadEvents = useCallback(
    async (betContract: any) => {
        ...
        try {
            ...
            const eventPromises = eventIdArray.map(async (eventIdBigInt) => {
            const eventId = Number(eventIdBigInt);
            ...
            try {
                ...
                console.error(`Error fetching contract details for event ${eventId}:`, error);
                return null;
            }
            });
    
            const resolvedEvents = (await Promise.all(eventPromises)).filter(Boolean);
    
            const now = Date.now() / 1000;
            const ongoing = resolvedEvents.filter(
            (e) => !e.isCompleted && Number(e.endTime) > now
            ).length;
    
            setEvents(resolvedEvents);
            setOngoingEventsCount(ongoing);
            setTotalEventsCount(resolvedEvents.length);
        } catch (err) {
            console.error("Error loading events list:", err);
            setError((prev) => prev || "Failed to load events list.");
            toast.error("Failed to load events list from contract.");
  ...
  const fetchRecentBets = useCallback(
    async (betContract: any, web3Instance: Web3) => {
        ...
        try {
            ...
            try {
                const block = await web3Instance.eth.getBlock(eventLog.blockNumber);
                const timestamp = typeof block.timestamp === "string" ? parseInt(block.timestamp, 10) : Number(block.timestamp);
                return {
                transactionHash: eventLog.transactionHash,
                eventId: String(eventLog.returnValues.eventId),
                bettor: eventLog.returnValues.bettor,
                amount: web3Instance.utils.fromWei(eventLog.returnValues.amount, "ether"),
                option: eventLog.returnValues.option,
            ...
            const resolvedBets = (await Promise.all(betsWithTimestampsPromises)).filter(Boolean) as RecentBet[];
            resolvedBets.sort((a, b) => b.timestamp - a.timestamp);
            setRecentBets(resolvedBets.slice(0, 20));
        } catch (err) {
            console.error("Error fetching recent bets:", err);
            setError((prev) => prev || "Failed to fetch recent bets.");
        } finally {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // --- REMOVED --- Obsolete raffle contract initialization
      // setRaffleContract(null);
        ...
        await Promise.all([
          (async () => {
            try {
              ...
              setAdminProfit(web3Instance.utils.fromWei(adminProfitWei, "ether"));
              setAdminAddress(adminAddr);
              setTotalBetsPlacedValue(web3Instance.utils.fromWei(totalBetsWei, "ether"));
            } catch (statsError) {
              console.error("Error fetching core contract stats:", statsError);
              setError((prev) => prev || "Failed to fetch contract stats.");
            }
  ...
  }, [fetchUserCount, loadEvents, fetchRecentBets]);
  ...
  // --- REMOVED --- Obsolete raffle event handlers
  // const handleRaffleCreated = useCallback(...);
  // const handleRaffleWinnerSelected = useCallback(...);

  // --- Loading & Error UI (No changes) ---
  const showInitialLoadingSkeleton = (loadingBlockchain || loadingUsers || loadingRecentBets) && !contract && !error;
  if (showInitialLoadingSkeleton) {
    return (
      <div className="flex h-screen bg-muted/40 dark:bg-primary">
        <Skeleton className="w-64 bg-card border-r border-gray-700/60 h-full" />
  ...
  if (error && !contract) {
    return (
      <div className="flex items-center justify-center h-screen flex-col bg-primary text-red-400 p-4">
        <h2 className="text-2xl font-semibold mb-4">Error Loading Admin Panel</h2>
        <p className="text-center mb-6">{error}</p>
        <Button variant="destructive" onClick={refreshAllData}>Retry Fetching Data</Button>
      </div>
    );
  }
      ...
      case "events":
        return contract && web3 ? (
          <EventsPage
            ...
            onWinnerDeclared={handleWinnerDeclared}
            onEventCreated={handleEventCreated}
            isLoading={loadingBlockchain && events.length === 0}
          />
        ) : <div className="p-6">Loading Events Data...</div>;
      
      // --- START OF CORRECTION ---
      case "raffles":
        // RafflesPage is now self-contained and does not require any props.
        return <RafflesPage />;
      // --- END OF CORRECTION ---
