...
const ResponsibleGamblingTab = ({ isLoggedin }: ResponsibleGamblingTabProps) => {
  // State for deposit limit dialog
  const [isDepositLimitDialogOpen, setIsDepositLimitDialogOpen] = useState(false)
  ...
  const handleSetLimitSubmit = async () => {
    ...
    try {
      ...
      const response = await responsibleGamblingService.setDepositLimits(limitData)

      if (response.success) {
        toast.success(`Your ${limitType} deposit limit has been set successfully`)
    ...
    >
      <div className="bg-[#333447] rounded-2xl p-6 mb-8 border-l-4 border-[#E27625] shadow-[0px_40px_80px_-20px_rgba(0,0,0,0.6)]">
        ...
        <p className="text-gray-300 text-sm">
          We're committed to promoting responsible gambling practices and providing tools to help you stay in control.
        </p>
        {!isLoggedin && (
          <div className="bg-red-500/20 border border-red-500/40 rounded-lg p-3 mt-4">
            <p className="text-red-200 text-sm">Please log in to access responsible gambling features.</p>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <Card className="bg-[#333447] overflow-hidden shadow-lg rounded-xl border-0 transition-all hover:shadow-xl">
          <CardContent className="p-6">
            ...
            <p className="text-sm text-gray-300 mb-6">
              Take control of your gambling by setting daily, weekly or monthly deposit limits.
            </p>
            <Button
        ...
        <Card className="bg-[#333447] overflow-hidden shadow-lg rounded-xl border-0 transition-all hover:shadow-xl">
          <CardContent className="p-6">
            ...
            <p className="text-sm text-gray-300 mb-6">
              Not sure if your gambling habits are becoming a problem? Take our confidential self-assessment.
            </p>
            <Button
              className="w-full bg-blue-500 hover:bg-blue-600 text-white"
              onClick={() => {
                if (!isLoggedin) {
                  toast.error("You must be logged in to take the self-assessment")
                  return
                }
        ...
        <Card className="bg-[#333447] overflow-hidden shadow-lg rounded-xl border-0 transition-all hover:shadow-xl">
          <CardContent className="p-6">
            ...
            <p className="text-sm text-gray-300 mb-6">
            Worried about your gambling or someone else's? Reach out to our support team for help.
            </p>
            <Button
              className="w-full bg-blue-500 hover:bg-blue-600 text-white"
              onClick={() => {
          ...
          <div className="grid gap-4">
            <div className="flex items-center justify-between p-3 bg-[#1C1C27] rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-medium">National Problem Gambling Helpline</span>
              </div>
              <span className="font-bold text-sm px-3 py-1 rounded-full">1-800-GAMBLER</span>
            </div>
            ...
            <div className="flex items-center justify-between p-3 bg-[#1C1C27] rounded-lg">
              ...
              <span className="font-bold text-sm px-3 py-1 rounded-full">1-626-960-3500</span>
            </div>
            ...
            <div className="flex items-center justify-between p-3 bg-[#1C1C27] rounded-lg">
              ...
              <span className="font-bold text-sm px-3 py-1 rounded-full">0808 8020 133</span>
            </div>
          </div>
        </div>
      </Card>
      ...
      <Dialog open={isDepositLimitDialogOpen} onOpenChange={setIsDepositLimitDialogOpen}>
        <DialogContent className="bg-[#1C1C27] border-0 p-0 overflow-hidden w-full max-w-md rounded-xl">
          <div className="bg-[#333447] p-6 border-b border-[#E27625]/20">
            <div className="flex items-center gap-3 mb-3">
              ...
              <DialogTitle className="text-xl font-bold">Set Deposit Limits</DialogTitle>
            </div>
            <DialogDescription className="text-gray-400 ml-11">
              Limit how much you can deposit in a given period to help manage your gambling.
            </DialogDescription>
          </div>
          ...
          <div className="p-6">
            <div className="space-y-6">
              <div>
                <Label htmlFor="limit-type" className="text-sm font-medium flex items-center gap-2 mb-2">
              ...
              <div>
                <Label htmlFor="limit-amount" className="text-sm font-medium flex items-center gap-2 mb-2">
                  <span className="inline-block w-2 h-2 rounded-full"></span>
                  Amount ($)
                </Label>
                ...
                <p className="text-xs text-gray-500 mt-2 ml-4">
                  {limitType === "daily" && "Maximum amount you can deposit in a 24-hour period"}
                  {limitType === "weekly" && "Maximum amount you can deposit in a 7-day period"}
                  {limitType === "monthly" && "Maximum amount you can deposit in a 30-day period"}
                </p>
              </div>
            </div>
