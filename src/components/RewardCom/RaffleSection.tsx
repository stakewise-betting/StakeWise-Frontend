...
import { Input } from "@/components/ui/input";
import { ConfirmationModal } from "./Confirmation-model";
import RaffleFilterBar from "./RaffleFilterBar";
...
const RaffleSection = () => {
    ...
    const loadRaffles = useCallback(async () => {
        ...
        try {
            ...
            setAllRaffles(visibleRaffles);
            setFilteredRaffles(visibleRaffles); // Initialize filtered raffles
    ...
    // Handle filter changes from RaffleFilterBar
    const handleFilterChange = useCallback((filtered: Raffle[]) => {
    ...
    // Get displayed raffles based on current view mode
    const displayedRaffles = showAllRaffles ? filteredRaffles : filteredRaffles.slice(0, 8);
            ...
            {filteredRaffles.length === 0 ? (
                <div className="text-center py-16 bg-gradient-to-br from-[#1a1b23] to-[#2a2b35] rounded-2xl border border-gray-700/50">
                    <div className="mb-6">
                        ...
                        <h3 className="text-2xl mb-3 font-bold text-white">
                            {allRaffles.length === 0 ? "No Active Raffles" : "No Matching Raffles"}
                        </h3>
                        <p className="text-gray-400 max-w-md mx-auto">
                            {allRaffles.length === 0 
                                ? "There are no active raffles at the moment. Check back soon for new opportunities to win amazing prizes!"
                                : "No raffles match your current filter criteria. Try adjusting your search or filter settings."
                            }
                        </p>
                    </div>
                </div>
                ...
                <>
                    ...
                    {!showAllRaffles && filteredRaffles.length > 8 && (
                        <div className="text-center mt-8">
                            <button 
                                onClick={handleToggleViewAll}
                                className="px-6 py-3 text-sm bg-transparent border-2 border-[#E27625] text-[#E27625] hover:bg-[#E27625] hover:text-white rounded-xl transition-all duration-300 font-semibold"
                            >
                                View All Raffles ({filteredRaffles.length})
                            </button>
                        </div>
                    )}
                </>
