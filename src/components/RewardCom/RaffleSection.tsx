...
//   // Fetch active raffles from your BACKEND API, not directly from the blockchain service.
//   useEffect(() => {
//     const fetchRafflesFromBackend = async () => {
//       setLoading(true);
//       try {
//         const response = await axios.get(`${backendUrl}/api/raffles/active`);
//         if (response.data.success) {
//           const activeRaffles = response.data.data;
//           // Sort raffles by end time (closest to ending first)
//           activeRaffles.sort((a: RaffleData, b: RaffleData) => a.endTime - b.endTime);
//           setRaffles(activeRaffles);
//         } else {
//           throw new Error(response.data.message || "Failed to fetch raffles from server.");
//         }
//       } catch (error) {
//         console.error("Error fetching raffles from backend:", error);
//         toast.error("Failed to load raffles. Please try again later.");
//       } finally {
//         setLoading(false);
//       }
...
//     fetchRafflesFromBackend();
//   }, [backendUrl]); // Depend on backendUrl

//   // Check which raffles the user is enrolled in.
//   // This part still needs to talk to the blockchain.
//   useEffect(() => {
//     const checkEnrollment = async () => {
//       if (isLoggedin && isConnected && walletAddress && raffles.length > 0) {
...
//     };
//     checkEnrollment();
//   }, [isLoggedin, isConnected, walletAddress, raffles]); // This now depends on the 'raffles' state

//   // --- THE REST OF THE LOGIC IS MOSTLY THE SAME ---

//   const handleBuyTicket = async (raffle: RaffleData) => {
//     // ... (Your existing logic for this function is good)
//   };
...
//   // ... (All other helper functions and JSX can remain the same)

//   // NOTE: I'm including the full component code below for easy copy-pasting,
//   // even though most of it is unchanged. The key change is the first useEffect.
...
//         <h2 className="text-2xl font-bold">Active Raffles</h2>
//         {isLoggedin && !isConnected && (
//           <Button onClick={connectWallet} disabled={isConnecting} className="mr-4 bg-[#E27625] hover:bg-[#d46222]">
//             {isConnecting ? <><Loader2Icon className="mr-2 h-4 w-4 animate-spin" />Connecting...</> : <><WalletIcon className="mr-2 h-4 w-4" />Connect Wallet</>}
//           </Button>
//         )}
//         <button className="px-4 py-2 text-sm bg-transparent border border-[#E27625] text-[#ffffff] hover:bg-[#E27625] rounded-lg transition">View All</button>
//       </div>
      
//       {loading ? (
//         <div className="flex justify-center items-center py-20"><Loader2Icon className="h-12 w-12 text-[#E27625] animate-spin" /></div>
//       ) : raffles.length === 0 ? (
//         <div className="text-center py-12 bg-[#333447] rounded-xl">
//           <h3 className="text-xl mb-2">No Active Raffles</h3>
//           <p className="text-gray-400">Check back soon for new opportunities!</p>
...
//         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
//           {raffles.map((raffle) => (
//             <div key={raffle.raffleId} className={`bg-[#333447] rounded-xl overflow-hidden border ${enrolledRaffles.includes(raffle.raffleId) ? "border-green-500 shadow-lg shadow-green-500/20" : "border-gray-700 hover:border-blue-500"} transition group`}>
//               <div className="relative h-48 overflow-hidden">
//                 <img src={raffle.imageURL || "/placeholder.svg"} alt={raffle.name} className="w-full h-full object-cover group-hover:scale-105 transition duration-500" />
//                 <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-t from-gray-900 to-transparent opacity-70"></div>
//                 {enrolledRaffles.includes(raffle.raffleId) ? (
//                   <div className="absolute top-4 right-4 bg-green-500 text-white px-3 py-1 rounded-full text-sm font-medium flex items-center"><CheckCircleIcon size={16} className="mr-1" />Enrolled</div>
//                 ) : (
//                   <div className="absolute bottom-4 left-4 bg-[#E27625] text-white px-3 py-1 rounded-full text-sm font-medium">Hot Deal</div>
//                 )}
//               </div>
//               <div className="p-5">
//                 <h3 className="text-xl font-bold mb-2">{raffle.name}</h3>
...
//                   <div className="text-center">
//                     <p className="text-sm text-gray-400">Prize Pool</p>
//                     <p className="text-xl font-bold text-[#00BD58]">{raffle.prizeAmount} ETH</p>
//                   </div>
//                 </div>
//                 <div className="flex justify-between mb-4">
//                   <div className="flex items-center"><ClockIcon size={16} className="text-gray-400 mr-1" /><span className="text-sm">Ends in {raffleBlockchainService.formatTimeLeft(raffle.endTime)}</span></div>
//                   <div className="flex items-center"><UsersIcon size={16} className="text-gray-400 mr-1" /><span className="text-sm">{raffle.totalTicketsSold} tickets</span></div>
//                 </div>
//                 <div className="flex items-center justify-between">
//                   <div className="flex items-center"><TicketIcon size={16} className="text-gray-400 mr-1" /><span className="text-sm">{raffle.ticketPrice} ETH</span></div>
//                   {enrolledRaffles.includes(raffle.raffleId) ? (
//                     <div className="flex items-center px-4 py-2 bg-gradient-to-r from-green-600 to-emerald-600 rounded-lg text-white"><TicketIcon size={16} className="mr-1" /><span>Ticket Purchased</span></div>
//                   ) : (
//                     <button className="flex items-center px-4 py-2 bg-gradient-to-r from-purple-600 to-blue-600 rounded-lg hover:opacity-90 transition" onClick={() => handleBuyTicket(raffle)}><CoinsIcon size={16} className="mr-1" /><span>Buy Ticket</span></button>
//                   )}
//                 </div>
//               </div>
//             </div>
...
//       <Dialog open={isPurchaseModalOpen} onOpenChange={setIsPurchaseModalOpen}>{/* ... */}</Dialog>
//       <ConfirmationModal open={isConfirmationModalOpen} onOpenChange={() => {}} raffle={selectedRaffle} ticketQuantity={ticketQuantity} />
//     </section>
//   );
// };
